...
//#endif
    ...
    /* Serialize a string or byte array to blindi */
    void serializeBytearrayToBlindi(SerializeOutput &output, const int32_t columnLength) const;
...
inline const NValue NValue::deserializeFromAllocateForStorage(SerializeInput &input, Pool *dataPool) {
    ...
    /**
     * Serializes a byte array for blindi. That means, it writes a full column and pads with zeroes the
     * bytes that are not used by the string or byte array. It also sets the preceding length to 0 so it won't affect
     * the comparison.
     */
    inline void NValue::serializeBytearrayToBlindi(SerializeOutput &output, const int32_t columnLength) const {
        const ValueType type = getValueType();

        if (type != VALUE_TYPE_VARCHAR && type != VALUE_TYPE_VARBINARY) {
            throwFatalException("Only string or byte array are supported.");
        }

        // write length - always 0 so it won't impact blindi comparison
        output.writeInt(0);

        // SQL NULL - write zeroes
        if (isNull()) {
            output.writeZeros(columnLength);
        } else {
            const int32_t length = getObjectLength();
            ...
            // main flow: not a null string, write it and pad it with zeroes
            if (length != OBJECTLENGTH_NULL) {
                const void *data = getObjectValue();

                if (data == NULL) {
                    throwFatalException("Failed to read string or bytearray");
                }

                output.writeBytes(data, length);
                output.writeZeros(columnLength - length);
            } else { // string is null - write zeroes
