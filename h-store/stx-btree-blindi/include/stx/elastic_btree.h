...
#ifndef _STX_BLINDI_ELASTIC_BTREE_H_HEADER_
#define _STX_BLINDI_ELASTIC_BTREE_H_HEADER_
...
#include "../blindi/blindi_seqtree_with_duplicates.hpp"

#define MILLION 1000000L
#define BILLION 1000L*MILLION

#define LIMIT_CAPACITY 50*MILLION
#define MUST_PERCENT 2500
...
namespace stx_blindi {

class NotImplementedWithBlindi : public std::logic_error
{
public:
    NotImplementedWithBlindi() : std::logic_error("Function was not modified to work with blindi."
                                                  "See commented-out lines for reference.") {};
};
...
public:
    ...
    static const int leafslots_prep = BTREE_MAX(8, 256 / (sizeof(_Key)));
    static const int leafslots = leafslots_prep - leafslots_prep % 2;

    /// Number of slots in each inner node of the tree. Estimated so that each node
    /// has a size of about 256 bytes.
    static const int innerslots_prep = BTREE_MAX(8, 256 / (sizeof(_Key) + sizeof(void*)));
    static const int innerslots = innerslots_prep - innerslots_prep % 2;
...
public:
    ...
    /// Number of slots in each leaf of the tree. Estimated so that each node
    /// has a size of about 256 bytes.
//    static const int leafslots_prep = BTREE_MAX(8, 256 / (sizeof(_Key) + sizeof(_Data)));
//    static const int leafslots = leafslots_prep - leafslots_prep % 2;

    // Token is used the change the leafslots using code generation.
 ...
 */
template <typename _Key, typename _Data,
          typename _KeyExtractor,
          typename _KeySerializer,
...
public:
    ...
    /// Define a one-byte allocator type. Used in blindi.
    ...
    static inline data_type& get_data_blindi(leaf_node *currnode, int currslot)
    {
        switch (currnode->blindileaftype)
        {
            case 1: {
                blindi_leaf_node1 *blindi_leaf1 = static_cast<blindi_leaf_node1*>(currnode);
                uint8_t * &blindi_data = blindi_leaf1->blindi_hybrid_node.get_data_reference(currslot);
                return reinterpret_cast<data_type &>(const_cast<const uint8_t * &>(blindi_data));
            }
            case 2: {
                blindi_leaf_node2 *blindi_leaf2 = static_cast<blindi_leaf_node2*>(currnode);
                uint8_t * &blindi_data = blindi_leaf2->blindi_hybrid_node.get_data_reference(currslot);
                return reinterpret_cast<data_type &>(const_cast<const uint8_t * &>(blindi_data));
            }
            case 4: {
                blindi_leaf_node4 *blindi_leaf4 = static_cast<blindi_leaf_node4*>(currnode);
                uint8_t * &blindi_data = blindi_leaf4->blindi_hybrid_node.get_data_reference(currslot);
                return reinterpret_cast<data_type &>(const_cast<const uint8_t * &>(blindi_data));
            }
            case 8: {
                blindi_leaf_node8 *blindi_leaf8 = static_cast<blindi_leaf_node8*>(currnode);
                uint8_t * &blindi_data = blindi_leaf8->blindi_hybrid_node.get_data_reference(currslot);
                return reinterpret_cast<data_type &>(const_cast<const uint8_t * &>(blindi_data));
            }
            default:
                std::cout << "Error: blindileaftype is " << currnode->blindileaftype << std::endl;
...
public:
    ...
    typedef btree<key_type, data_type, key_extractor_type, key_serializer_type, value_type, key_compare,
...
private:
...
#endif
        /// Delayed initialisation of constructed node
        inline node(const unsigned short l, const uint8_t bln)
            : level(l), slotuse(0), blindileaftype(bln)
        { }
    ...
    struct leaf_node : public node
    {
        ...
        /// Set variables to initial values
        inline leaf_node(const uint8_t bln=0)
            : node(0, bln), prevleaf(NULL), nextleaf(NULL),
        { }
        ...
        {
            if (node::blindileaftype == BLINDI_MAX_MULTI_SIZE)
                return 0;
            return 1;
        }
    ...
    struct btree_leaf_node : public leaf_node
    {
        ...
        /// Set variables to initial values
        inline leaf_node()
            : leaf_node(0), prevleaf(NULL), nextleaf(NULL),
        { }
    ...
    {
        // define the inner node type
        typedef GenericBlindiSeqTreeNode <BlindiBtreeHybridType<key_type, leafslots>, key_type, leafslots, key_extractor_type, key_serializer_type, allocator_type> blindi_node1;
        ...
        /// Set variables to initial values
        inline blindi_leaf_node1(key_extractor_type & key_extractor, key_serializer_type & key_serializer, byte_alloc_type & byte_allocator)
            : leaf_node(1), blindi_bybrid_node(key_extractor, key_serializer, byte_allocator)
        { }
    };
    ...
    {
        // define the inner node type
        typedef GenericBlindiSeqTreeNode <BlindiBtreeHybridType<key_type, leafslots*2>, key_type, leafslots*2, key_extractor_type, key_serializer_type, allocator_type> blindi_node2;
        ...
        /// Set variables to initial values
        inline blindi_leaf_node2(key_extractor_type & key_extractor, key_serializer_type & key_serializer, byte_alloc_type & byte_allocator)
            : leaf_node(2), blindi_bybrid_node(key_extractor, key_serializer, byte_allocator)
        { }
    };
    ...
    {
        // define the inner node type
        typedef GenericBlindiSeqTreeNode <BlindiBtreeHybridType<key_type, leafslots*4>, key_type, leafslots*4, key_extractor_type, key_serializer_type, allocator_type> blindi_node4;
        ...
        /// Set variables to initial values
        inline blindi_leaf_node4(key_extractor_type & key_extractor, key_serializer_type & key_serializer, byte_alloc_type & byte_allocator)
            : leaf_node(4), blindi_bybrid_node(key_extractor, key_serializer, byte_allocator)
        { }
    };
    ...
    {
        // define the inner node type
        typedef GenericBlindiSeqTreeNode <BlindiBtreeHybridType<key_type, leafslots*8>, key_type, leafslots*8, key_extractor_type, key_serializer_type, allocator_type> blindi_node8;
        ...
        /// Set variables to initial values
        inline blindi_leaf_node8(key_extractor_type & key_extractor, key_serializer_type & key_serializer, byte_alloc_type & byte_allocator)
            : leaf_node(8), blindi_bybrid_node(key_extractor, key_serializer, byte_allocator)
        { }
    };
...
public:
    ...
    public:
        ...
        /// The key extractor type of the btree.
        typedef typename btree::key_extractor_type key_extractor_type;

        /// The key serializer type of the btree.
        typedef typename btree::key_serializer_type key_serializer_type;
    ...
    private:
        ...
        friend class btree<key_type, data_type, key_extractor_type, key_serializer_type, value_type, key_compare,
    ...
    public:
        ...
        inline const key_type key() const
        {
            if (currnode->isblindileafnode())
                return get_key_blindi(currnode, currslot);
            else {
        ...
        {
            int idx = used_as_set ? 0 : currslot;

            if (currnode->isblindileafnode()) {
                return get_data_blindi(currnode, idx);
            } else {
                 btree_leaf_node *btree_leaf = static_cast<btree_leaf_node*>(currnode);
                 return btree_leaf->slotdata[idx];
            }
        }
    ...
    public:
        ...
        inline const key_type key() const
        {
            ...
            if (currnode->level == 0 && currnode->sblindileafnode()) {
                return get_key_blindi(currnode, currslot - 1);
            }
        ...
        inline data_type & data() const
        {
            ...
            int idx = used_as_set ? 0 : currslot - 1;

            if (currnode->level == 0 && currnode->isblindileafnode()) {
                return get_data_blindi(currnode, idx);
            }
            return currnode->slotkey[idx];
        }
...
//              itemcount(0),
              ...
              limit2MustCompress(uint64_t(LIMIT_CAPACITY /100 * MUST_PERCENT)),
...
private:
    ...
    /// Byte allocator for blindI
    byte_alloc_type m_byte_allocator;
...
public:
    ...
    /// comparison function
    explicit inline btree(key_extractor_type key_extractor,
                                              key_serializer_type key_serializer,
                                              const allocator_type &alloc = allocator_type())
        : m_root(NULL), m_headleaf(NULL), m_tailleaf(NULL), m_allocator(alloc),  m_key_extractor(key_extractor), m_key_serializer(key_serializer), m_byte_allocator(alloc)
    { }
    ...
    explicit inline btree(const key_compare& kcf,
                          ...
                          key_serializer_type key_serializer)
        : m_root(NULL), m_headleaf(NULL), m_tailleaf(NULL),
          m_key_less(kcf), m_allocator(alloc), m_key_extractor(key_extractor), m_key_serializer(key_serializer), m_byte_allocator(alloc)
    { }
    ...
    inline btree(InputIterator first, InputIterator last,
                                     ...
                                     key_serializer_type key_serializer,
                                     const allocator_type &alloc = allocator_type())
        : m_root(NULL), m_headleaf(NULL), m_tailleaf(NULL), m_key_extractor(key_extractor),m_key_serializer(key_serializer), m_allocator(alloc), m_byte_allocator(alloc)
    {
        insert(first, last);
    }
...
private:
    ...
    {
        btree_leaf_node* n = new (btree_leaf_node_allocator().allocate(1))
                             btree_leaf_node();
#ifdef BREATHING_SIZE_BTREE
#ifndef BREATHING_DATA_ONLY_BTREE
        n->slotkey = (key_type *) m_byte_allocator(slotsize * sizeof(key_type));
#endif 
        n->slotdata = (data_type *) m_byte_allocator(slotsize * sizeof(data_type));
    ...
    {
        blindi_leaf_node1* n = new (btree_leaf_node1_allocator().allocate(1))
                               blindi_leaf_node1(m_key_extractor, m_key_serializer, m_byte_allocator);
        m_stats.leaves++;
    ...
    {
        blindi_leaf_node2* n = new (btree_leaf_node2_allocator().allocate(1))
                               blindi_leaf_node2(m_key_extractor, m_key_serializer, m_byte_allocator);
        m_stats.leaves++;
        m_stats.blindileaves++;
    ...
    {
        blindi_leaf_node4* n = new (btree_leaf_node4_allocator().allocate(1))
                               blindi_leaf_node4(m_key_extractor, m_key_serializer, m_byte_allocator);
        m_stats.leaves++;
        m_stats.blindileaves++;
    ...
    {
        blindi_leaf_node8* n = new (btree_leaf_node8_allocator().allocate(1))
                               blindi_leaf_node8(m_key_extractor, m_key_serializer, m_byte_allocator);
        m_stats.leaves++;
        m_stats.blindileaves++;
...
private:
    ...
    inline int find_lower_in_leaf(const struct leaf_node * n, const key_type & key) const {
        int lo = n->inner_blindi_node.PredecessorSearchWrapper((uint8_t *) &key);

        while (lo < n->slotuse && key_less(n->inner_blindi_node.get_key(lo), key)) ++lo;
    ...
    inline int find_upper_in_leaf(const struct leaf_node * n, const key_type & key) const {
        int lo = n->inner_blindi_node.PredecessorSearchWrapper((uint8_t *) &key);

        while (lo < n->slotuse && key_lessequal(n->inner_blindi_node.get_key(lo), key)) ++lo;
...
public:
    // BLINDI FUNCTUIONS 
    static inline int search_blindi(leaf_node *currnode, const key_type &key) const
    {
        switch (currnode->blindileaftype) 
        {
            case 1: {
                blindi_leaf_node1 *blindi_leaf1 = static_cast<blindi_leaf_node1*>(currnode);
                return blindi_leaf1->blindi_hybrid_node.SearchBlindiNodeWrapper((uint8_t *) &key);
            }
            case 2: {
                blindi_leaf_node2 *blindi_leaf2 = static_cast<blindi_leaf_node2*>(currnode);
                return blindi_leaf2->blindi_hybrid_node.SearchBlindiNodeWrapper((uint8_t *) &key);
            }
            case 4: {
                blindi_leaf_node1 *blindi_leaf4 = static_cast<blindi_leaf_node4*>(currnode);
                return blindi_leaf4->blindi_hybrid_node.SearchBlindiNodeWrapper((uint8_t *) &key);
            }
            case 8: {
                blindi_leaf_node8 *blindi_leaf8 = static_cast<blindi_leaf_node8*>(currnode);
                return blindi_leaf8->blindi_hybrid_node.SearchBlindiNodeWrapper((uint8_t *) &key);
            }
            default:
                std::cout << "Error: blindileaftype is " << currnode->blindileaftype << std::endl;
                assert(0);	    
    ...
    static inline int insert_blindi(leaf_node *currnode, const key_type &key) const
    {
        switch (currnode->blindileaftype) 
        {
            case 1: { 
    ...
    static inline void split_blindi_nodex(leaf_node *currnode, key_type* splitkey, node ** splitnode)
    {
        switch (currnode->blindileaftype) 
        {
            case 1: { 
    ...
    bool exists(const key_type& key) const
    {
        ...
        // blindi_leaf_node
        int blindi_slot = search_blindi(leaf, key);
        return blindi_slot < leaf->slotuse;
    }
    ...
    iterator find(const key_type& key)
    {
        ...
        return (blindi_slot < leaf->slotuse) ? iterator(leaf, blindi_slot) : end();
    }
    ...
    size_type count(const key_type& key) const
    {
        ...
        slot = find_lower_in_leaf(leaf, key);
        size_type num = 0;

        while (leaf && slot < leaf->slotuse && key_equal(key, leaf->inner_blindi_node.get_key(slot)))
        {
            ++num;
    ...
    iterator lower_bound(const key_type& key)
    {
        ...
        // blindi_leaf_node1
        int slot = find_lower_in_leaf(leaf, key);
        return iterator(leaf, slot);
    }
    ...
    const_iterator lower_bound(const key_type& key) const
    {
        ...
        const leaf_node* leaf = static_cast<leaf_node*>(n);
    ...
    iterator upper_bound(const key_type& key)
    {
        ...
        int slot = find_upper_in_leaf(leaf, key);
        return iterator(leaf, slot);
    }
...
private:
    ...
    std::pair<iterator, bool> insert_start(const key_type& key, const data_type& value)
    {
        ...
        std::pair<iterator, bool> r = insert_descend(m_root, key, value, &newkey, &newchild, m_root, 0);
    ...
    */
    std::pair<iterator, bool> insert_descend(node* n,
                                             ...
                                             key_type* splitkey, node** splitnode, node* parent_n, int parentslot)
    {
        if (!n->isleafnode())
        {
            ...
            std::pair<iterator, bool> r = insert_descend(inner->childid[slot],
                                                         key, value, &newkey, &newchild, n, slot);
    ...
    void connect_newleaf2tree(node *parent_n, int parentslot, leaf_node *old_leaf, leaf_node *new_leaf)
    {
        ...
        // change the parent pointer/ change the left pointer and the right pointer
    ...
    void split_btree_leaf_node(btree_leaf_node* leaf, key_type* _newkey, node** _newleaf)
    {
        ...
        BTREE_PRINT("btree::split_btree_leaf_node on " << leaf);
#ifdef BREATHING_SIZE_BTREE
    ...
    void transfer_blindi_node_btree_node(blindi_leaf_node1 *blindileaf, btree_leaf_node *btreeleaf)
    {
        ...
        for (int i=0; i < blindileaf->slotuse; i++) {  
            btreeleaf->slotdata[i] = get_data_blindi(blindileaf, i);
            btreeleaf->slotkey[i] = get_key_blindi(blindileaf, i);
        }
    }
    ...
    static void print_node(std::ostream& os, const node* node, unsigned int depth = 0, bool recursive = false)
    {
	    ...
	    else 
	    {
		    os << "blindi_leaf_node("<<(int) leafnode->blindileaftype <<") " <<  std::endl;
		     ...
		     for (unsigned int slot = 0; slot < leafnode->slotuse; ++slot)
		     {
			     print_key (os, get_key_blindi(leafnode, slot));
		     }
		     os << std::endl;
	    }
        }
	    ...
	    { 		 
		    ...
		    if( leaf != m_root && leaf->slotuse == 0 ){
			    ...
			    {	
				    ...
				    _small_key = get_key_blindi(leaf, slot);
				    _large_key = get_key_blindi(leaf, slot + 1);
...
*/
    ...
    void verify_leaflinks() const
    {
        ...
        while (n)
        {
            ...
            {
                assert(key_lessequal(n->inner_blindi_node.get_key(slot), n->inner_blindi_node.get_key(slot + 1)));
            }
            ...
            if (n->nextleaf)
            {
                assert(key_lessequal(n->inner_blindi_node.get_key(n->slotuse - 1), n->nextleaf->inner_blindi_node.get_key(0)));
...
#endif // !_STX_BLINDI_ELASTIC_BTREE_H_HEADER_
