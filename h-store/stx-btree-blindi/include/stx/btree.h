...
#ifndef _STX_BLINDI_BTREE_H_HEADER_
#define _STX_BLINDI_BTREE_H_HEADER_
...
private:
    ...
    struct leaf_node : public node
    {
        ...
        typedef GenericBlindiSeqTreeNode <key_type, leafslotmax, key_extractor_type, key_serializer_type, allocator_type> blindi_node;

        // BlindI inner node
        blindi_node inner_blindi_node;
        ...
        /// Set variables to initial values
        inline leaf_node(key_extractor_type & key_extractor, key_serializer_type & key_serializer, byte_alloc_type & byte_allocator)
            : node(0), prevleaf(NULL), nextleaf(NULL), inner_blindi_node(key_extractor, key_serializer, byte_allocator)
        { }
...
public:
    ...
    public:
        ...
        {
            key_type blindi_key = currnode->inner_blindi_node.get_key(currslot);
        ...
        inline data_type & data() const
        {
            ...
            uint8_t * &blindi_data = currnode->inner_blindi_node.get_data_reference(idx);

            return (data_type &)(blindi_data);
        }
    ...
    public:
        ...
        inline const data_type & data() const
        {
            ...
            auto node = const_cast<typename btree::leaf_node *>(currnode);
            uint8_t * &blindi_data = node->inner_blindi_node.get_data_reference(idx);
    ...
    public:
        ...
        inline const key_type key() const
        {
            ...
            key_type blindi_key = currnode->inner_blindi_node.get_key(currslot - 1);
...
private:
    ...
    inline leaf_node * allocate_leaf()
    {
        leaf_node* n = new (leaf_node_allocator().allocate(1))
                       leaf_node(m_key_extractor, m_key_serializer, m_byte_allocator);
        m_stats.leaves++;
        return n;
    }
...
public:
    // *** Utility functions for memory consumption (theoretic vs actual) checks.
    inline int64_t get_theoretic_size() const{
        return m_stats.leaves * sizeof(leaf_node)
               + m_stats.innernodes * sizeof(inner_node);
    }

    inline int64_t get_leaf_size() const{
        return sizeof(leaf_node);
    }

    inline int64_t get_inner_node_size() const{
        return sizeof(inner_node);
    }

    inline int get_inner_node_item_count() const{
        return traits::innerslots;
    }
    inline int get_leaf_item_count() const{
        return leafslotmax;
    }
    inline int64_t get_key_size() const{
...
public:
    ...
    bool exists(const key_type& key) const
    {
        ...
        // BlindI search
        int blindi_slot = leaf->inner_blindi_node.SearchBlindiNodeWrapper((uint8_t *) &key);
...
private:
    ...
    struct node * copy_recursive(const node* n)
    {
        if (n->isleafnode())
        {
            ...
            newleaf->inner_blindi_node.node.allocate();
            newleaf->inner_blindi_node.node.CopyNode(&(leaf->inner_blindi_node.node));
    ...
    */
                                             ...
                                             key_type* splitkey, node** splitnode)
    {
        ...
        else // n->isleafnode() == true
        {
            ...
            // BlindI search
            int slot = leaf->inner_blindi_node.SearchBlindiNodeWrapper((uint8_t *) (&key));
            ...
            if (leaf->isfull())
            {
                ...
                // check if insert slot is in the split sibling node
                if (key_greater(key, leaf->inner_blindi_node.get_max_key_in_node())) {
                    leaf = static_cast<leaf_node*>(*splitnode);
                }
            }

            /* Insert into the blindI inner node.
             * Note that I would like to write something simple like this:
             *
             * INSERT_RESULT res = leaf->inner_blindi_node.Insert2BlindiNodeWithKey((void *) value));
             *
             * But it won't compile due to the difference in static types and cv qualifiers
             * between btree and blindi.
             */
            auto p_data = reinterpret_cast<const uint8_t * const *>(&value);
            uint8_t * data = const_cast<uint8_t *>(*p_data);

            INSERT_RESULT res = leaf->inner_blindi_node.Insert2BlindiNodeWithKey((uint8_t *) (&key), data);

            switch (res) {
                case INSERT_SUCCESS:
                case INSERT_DUPLICATED_KEY:
//                    BTREE_PRINT("BlindI: insertion succeeded");
                    break;
                    break;
                case INSERT_OVERFLOW:
                    BTREE_PRINT("BlindI: insertion failed: overflow");
                    break;
            }

            leaf->slotuse++;

            slot = leaf->inner_blindi_node.SearchBlindiNodeWrapper((uint8_t *) &key);
            ...
            if (*splitnode && leaf != *splitnode && slot == leaf->slotuse - 1)
            {
                ...
                *splitkey = key;
                ...
                // TODO: handle duplicates, if required. Anyway, I think that this is dead code,
                // since in this case, the key will always be inserted to the split-node
                //printf("This is not a dead code!\n");
            }
    ...
    void split_leaf_node(leaf_node* leaf, key_type* _newkey, node** _newleaf)
    {
        ...
        // BlindI split
        leaf->inner_blindi_node.SplitBlindiNode(&newleaf->inner_blindi_node);
        ...
        // blindi assertions
        BTREE_ASSERT(leaf->slotuse == leaf->inner_blindi_node.get_valid_len());
        BTREE_ASSERT(newleaf->slotuse == newleaf->inner_blindi_node.get_valid_len());

        *_newkey = leaf->inner_blindi_node.get_max_key_in_node();
        *_newleaf = newleaf;
    }
...
private:
                               ...
                               inner_node* parent, unsigned int parentslot)
    {
        if (curr->isleafnode())
        {
            ...
            if (slot >= leaf->slotuse || !key_equal(key, leaf->inner_blindi_node.get_key(slot)))
            {
            ...
            REMOVE_RESULT blindi_res = leaf->inner_blindi_node.RemoveFromBlindiNodeInPosition(slot);
            BTREE_ASSERT(blindi_res != REMOVE_KEY_NOT_FOUND);
            ...
            if (slot == leaf->slotuse)
            {
                if (parent && parentslot < parent->slotuse)
                {
                    ...
                    parent->slotkey[parentslot] = leaf->inner_blindi_node.get_key(leaf->slotuse - 1);
                }
                else
                {
                    if (leaf->slotuse >= 1)
                    {
                        BTREE_PRINT("Scheduling lastkeyupdate: key " << leaf->inner_blindi_node.get_key(leaf->slotuse - 1));
                        myres |= result_t(btree_update_lastkey, leaf->inner_blindi_node.get_key(leaf->slotuse - 1));
                    }
                    else
                    {
            ...
            if (leaf->isunderflow() && !(leaf == m_root && leaf->slotuse >= 1))
            {
                ...
                BTREE_ASSERT(blindi_res == REMOVE_UNDERFLOW || blindi_res == REMOVE_NODE_EMPTY);
        ...
        else // !curr->isleafnode()
        {
            ...
            if (result.has(btree_fixmerge))
            {
                ...
                if (inner->level == 1)
                {
                    ...
                    inner->slotkey[slot] = child->inner_blindi_node.get_key(child->slotuse - 1);
                }
            }
    ...
    result_t merge_leaves(leaf_node* left, leaf_node* right, inner_node* parent)
    {
        ...
        left->inner_blindi_node.MergeBlindiNodes(&right->inner_blindi_node);
    ...
    static result_t shift_left_leaf(leaf_node* left, leaf_node* right, inner_node* parent, unsigned int parentslot)
    {
        ...
        left->inner_blindi_node.ShiftLeftLeaf(left->inner_blindi_node, right->inner_blindi_node, shiftnum);
        left->slotuse += shiftnum;
        right->slotuse -= shiftnum;

        // fixup parent
        if (parentslot < parent->slotuse) {
            parent->slotkey[parentslot] = left->inner_blindi_node.get_key(left->slotuse - 1);
            return result_t(btree_ok);
        }
        else {  // the update is further up the tree
            return result_t(btree_update_lastkey, left->inner_blindi_node.get_key(left->slotuse - 1));
        }
    }
    ...
    static void shift_right_leaf(leaf_node* left, leaf_node* right, inner_node* parent, unsigned int parentslot)
    {
        ...
        left->inner_blindi_node.ShiftRightLeaf(left->inner_blindi_node, right->inner_blindi_node, shiftnum);
        right->slotuse += shiftnum;
        left->slotuse -= shiftnum;
...
private:
    ...
    void verify_node(const node* n, key_type* minkey, key_type* maxkey, tree_stats& vstats) const
    {
        ...
        if (n->isleafnode())
        {
            ...
            {
                assert(key_lessequal(leaf->inner_blindi_node.get_key(slot), leaf->inner_blindi_node.get_key(slot + 1)));
            }

            *minkey = leaf->inner_blindi_node.get_min_key_in_node();
            *maxkey = leaf->inner_blindi_node.get_max_key_in_node();
...
#endif // !_STX_BLINDI_BTREE_H_HEADER_
