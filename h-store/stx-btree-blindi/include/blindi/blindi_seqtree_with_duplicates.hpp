...
//BLINDI_BREATH_SIZE_ID_STR
#define BREATHING_BLINDI_SIZE 4
//BLINDI_BREATH_DATA_ONLY_ID_STR
...
#ifndef TREE_LEVELS
//TREE_LEVEL_ID_STR
#define TREE_LEVELS 0 // maximum 8
#endif

#define TREE_SIZE (1 << (TREE_LEVELS)) - 1
#define END_TREE ((idx_t)-1)
#define DUPLICATE ((bitidx_t)-1)
#define MAX_HIGH_MISS ((bitidx_t)-2)
//#define DEBUG_AGG
...
// A BlindI node
template <typename _Key, int NUM_SLOTS, typename _KeyExtractor, typename _KeySerializer, typename _Alloc>
class SeqTreeBlindiNode {
    typedef _KeyExtractor key_extractor_type;
    typedef _KeySerializer key_serializer_type;

    /// Define a one-byte allocator type.
    typedef typename _Alloc::template rebind<char>::other byte_alloc_type;

    key_extractor_type & m_key_extractor;
    key_serializer_type & m_key_serializer;
    byte_alloc_type & m_allocator;

    static_assert((NUM_SLOTS % 2) == 0, "NUM_SLOTS is not even");

#if (TREE_LEVELS > 0)
	//    static_assert((NUM_SLOTS <= 254), "NUM_SLOTS > 254");
#else
    ...
    public:
    SeqTreeBlindiNode(
            key_extractor_type &key_extractor, key_serializer_type &key_serializer,
            byte_alloc_type &alloc
    ) :
            m_key_extractor(key_extractor), m_key_serializer(key_serializer), m_allocator(alloc) {
        valid_len = 0;
    }   // Constructor

    key_serializer_type & get_key_serializer() const {
        return this->m_key_serializer;
    }
    ...
    uint8_t * get_data(int pos) const {
        return this->key_ptr[pos];
    }

    _Key get_key(int pos) const {
        return m_key_extractor.extractKey(reinterpret_cast<char *>(this->key_ptr[pos]));
    }

    uint8_t * & get_data_reference(int pos) {
        return this->key_ptr[pos];
    }
	...
	uint16_t *get_ptr2blindiTree(){
#if (TREE_LEVELS > 0)
		return this->blindiTree;
#else
		return NULL;
#endif
	}
...
#ifdef BREATHING_BLINDI_SIZE
	inline bool is_need_breath() const {
        return (this->valid_len == this->currentmaxslot);
    }

    inline bool will_need_breath(unsigned int new_items_count) const {
        return (this->valid_len + new_items_count > this->currentmaxslot);
    }

    void breath() {
        uint16_t oldmaxslot = currentmaxslot;
        ...
        uint8_t **tmp_key_ptr = (uint8_t **)m_allocator.allocate (currentmaxslot * sizeof(uint8_t *));
        std::copy(key_ptr, key_ptr + valid_len, tmp_key_ptr);
        m_allocator.deallocate(reinterpret_cast<char *>(key_ptr), oldmaxslot * sizeof(uint8_t *));
        key_ptr = tmp_key_ptr;

#ifndef BREATHING_BLINDI_DATA_ONLY
        bitidx_t *tmp_blindiKeys = (bitidx_t *)m_allocator.allocate ((currentmaxslot - 1) * sizeof(bitidx_t));
        std::copy(blindiKeys, blindiKeys + valid_len - 1,
                  tmp_blindiKeys);
        m_allocator.deallocate(reinterpret_cast<char *>(blindiKeys), (oldmaxslot - 1) * sizeof(bitidx_t));
    ...
    /**
	 * Breathing-only:
	 * Deallocate the blindi node - key_ptr and blindiKeys.
	 */
	inline void deallocate() {
        m_allocator.deallocate(reinterpret_cast<char *>(key_ptr), currentmaxslot * sizeof(uint8_t *));
#ifndef BREATHING_BLINDI_DATA_ONLY
        m_allocator.deallocate(reinterpret_cast<char *>(blindiKeys), (currentmaxslot - 1) * sizeof(bitidx_t));
#endif
    }
    ...
    /**
     * Sets this->valid_len = 0;
     * When breathing is enabled, allocates key_ptr and blindiKeys.
     */
...
breathing_count++;
...
#ifdef BREATHING_BLINDI_SIZE
        currentmaxslot = (uint16_t) NUM_SLOTS;
        key_ptr = (uint8_t **) m_allocator.allocate(NUM_SLOTS * sizeof(uint8_t *));
#ifndef BREATHING_BLINDI_DATA_ONLY
        this->blindiKeys = (bitidx_t *)m_allocator.allocate((NUM_SLOTS - 1) * sizeof(bitidx_t));
#endif
#endif
    }
...
#if (TREE_LEVELS > 0)
	static void bring_edges_seq_from_pos(idx_t *blindiTree, int valid_len, int pos, int *start_pos, int *end_pos)
	{
		switch (pos) {
			case 0 :
				*start_pos = 0;
	...
	uint16_t SearchFirstStage (const uint8_t *key_ptr, uint16_t key_len,  uint16_t *tree_traverse, uint16_t *tree_traverse_len) const {
	...
	int SearchSecondStageSequence (int first_stage_pos, int diff_pos, bool large_small, bool *smaller_than_node, uint16_t* tree_traverse, uint16_t *tree_traverse_len, bool len_diff = 0) const {
	...
	int SearchSecondStageTree (int diff_pos, bool large_small, bool is_pointer_area, bool *smaller_than_node, uint16_t* tree_traverse, uint16_t *tree_traverse_len, bool len_diff = 0) const {
		int temp;
		int i;
...
#if (TREE_LEVELS > 1)
			int temp[TREE_SIZE] = {0};
			// switch tree_traverse 2
...
#if (TREE_LEVELS > 0)
    static void set_blindiTree_from_seq(SeqTreeBlindiNode * node) {
        node->blindiTree[0] = 0;

        for (int i=0; i < (node->valid_len - 1); i++) {
            node->blindiTree[0] = (node->blindiKeys[i] < node->blindiKeys[node->blindiTree[0]]) ? i
                                                                                                : node->blindiTree[0];
        }
        ...
        for (int l = 2; l <= TREE_LEVELS; l++) {
            for (int i = pow(2, (l - 1)) - 1; i <= pow(2, l) - 2; i++) {
                // if my parent does not END_TREE update the value
                if (node->blindiTree[i >> 1] != END_TREE) {
                    update_blindiTree_from_seq(node, i);
                } else // the parent is END_TREE so also I get END_TREE
                {
                    node->blindiTree[i] = END_TREE;
                }
            }
        }
    #endif
	...
	static void update_blindiTree_from_tree (SeqTreeBlindiNode *target_node, int pos, idx_t* point_orig_2blindiTree, idx_t* orig_blindiTree, int half_node_size = 0) {
		if (target_node->blindiTree[0] == END_TREE) {
			target_node->blindiTree[0] = orig_blindiTree[pos] - half_node_size;
			point_orig_2blindiTree[0] = orig_blindiTree[pos];
			return;
		}
		int cur_pos = (orig_blindiTree[pos] < point_orig_2blindiTree[0]) ? 1 : 2;
		for (int l = 2; l <= TREE_LEVELS; l++) {
			if (target_node->blindiTree[cur_pos] == END_TREE) {
				target_node->blindiTree[cur_pos] = orig_blindiTree[pos] - half_node_size;
				point_orig_2blindiTree[cur_pos] = orig_blindiTree[pos];
				return;
			}
			cur_pos =  orig_blindiTree[pos] < point_orig_2blindiTree[cur_pos] ? ((cur_pos  + 1 )<< 1) - 1 : ((cur_pos + 1) << 1);
		}
		assert(0);
	}


	static int min_pos_value_in_range(SeqTreeBlindiNode *node, int start_pos, int end_pos) {
	...
	static void update_blindiTree_from_seq (SeqTreeBlindiNode *node, int pos) {
		int start_pos;
		int end_pos;
	...
	// SEQ part: uses this node as small, and node received as parameter as large
	//  1. (0 - NUM_SLOTS /2 -1) -> copy it to node_small (this node)
	//  2. (NUM_SLOTS/2 : NUM_SLOTS -1) -> copy it to node_large
	// TREE part : create two new trees

	void SplitBlindiNode(SeqTreeBlindiNode *node_large) {
        SeqTreeBlindiNode *node_small = this;

#ifdef BREATHING_BLINDI_SIZE
		uint16_t cms;
        uint16_t oldmaxslot = node_small->currentmaxslot;
		...
		node_large->key_ptr = (uint8_t **)m_allocator.allocate (cms * sizeof(uint8_t *));
#ifndef BREATHING_BLINDI_DATA_ONLY
		node_large->blindiKeys = (bitidx_t *)m_allocator.allocate ((cms - 1) * sizeof(bitidx_t));
#endif 
#endif

		// Update the new node and the original_node
...
#if (TREE_LEVELS > 0)
        idx_t orig_blindiTree[TREE_SIZE];

        for (int i = 0; i < TREE_SIZE; i++) {
            orig_blindiTree[i] = node_small->blindiTree[i];
        }
		...
		if (orig_blindiTree[0] <= half_node_size - 2) {
			update_blindiTree_from_tree(node_small, 0, node_small_point_orig_blindiTree, orig_blindiTree);
		} else if (orig_blindiTree[0] >= half_node_size && orig_blindiTree[0] != END_TREE) {
			update_blindiTree_from_tree(node_large, 0, node_large_point_orig_blindiTree, orig_blindiTree, half_node_size);
		}

		for (int l = 2; l <= TREE_LEVELS; l++) {
			for (int i = pow(2,(l -1)) - 1; i <= pow(2,l) - 2; i++) {
				if (this->blindiTree[((i + 1) >> 1) - 1] != END_TREE )  {
					if (orig_blindiTree[i] <= half_node_size - 2) {
						update_blindiTree_from_tree(node_small ,i, node_small_point_orig_blindiTree, orig_blindiTree);
					} else if (orig_blindiTree[i] >= half_node_size && orig_blindiTree[i] != END_TREE) {
						update_blindiTree_from_tree(node_large, i, node_large_point_orig_blindiTree, orig_blindiTree, half_node_size);
					}
				}
			}
		}
...
#endif
#ifdef BREATHING_BLINDI_SIZE
        uint8_t ** old_key_ptr = node_small->key_ptr;

		// realloc
        node_small->key_ptr = (uint8_t **)m_allocator.allocate(cms * sizeof(uint8_t *));
        std::copy(old_key_ptr, old_key_ptr + half_node_size, node_small->key_ptr);
        m_allocator.deallocate(reinterpret_cast<char *>(old_key_ptr), oldmaxslot * sizeof(uint8_t *));
#ifndef BREATHING_BLINDI_DATA_ONLY
        bitidx_t * old_blindiKeys = node_small->blindiKeys;

        // realloc
        node_small->blindiKeys = (bitidx_t *)m_allocator.allocate((cms - 1) * sizeof(bitidx_t));
        std::copy(old_blindiKeys, old_blindiKeys + half_node_size - 1, node_small->blindiKeys);
        m_allocator.deallocate(reinterpret_cast<char *>(old_blindiKeys), (oldmaxslot - 1) * sizeof(bitidx_t));
#endif 
#endif
	}
	//  Remove shift key_ptr and shift BlindiKeys, & valid_len -= 1
    REMOVE_RESULT RemoveFromBlindiNodeInPosition(uint16_t remove_position) {
		// SEQ
    ...
    uint16_t CalculateBlindiKey(const uint8_t *key1, const uint8_t *key2) const {
        bool _hit, _large_small;
        uint16_t _diff_pos;

        // serialize the keys
        uint8_t serialized_small_key[sizeof(_Key)];
        uint8_t serialized_large_key[sizeof(_Key)];
        m_key_serializer.serialize(key1, serialized_small_key);
        m_key_serializer.serialize(key2, serialized_large_key);

        _diff_pos = CompareStage (serialized_small_key, serialized_large_key, &_hit, &_large_small, sizeof(_Key), sizeof(_Key));
        if (_hit)
        {
            _diff_pos = DUPLICATE;
        }
...
// The merge is from the large node to small_node (this)
	void MergeBlindiNodes(SeqTreeBlindiNode *node_large) {
#ifdef DEBUG_SEQTREE
			std::cout << " before Merge " << std::endl;
			print_node();
			node_large->print_node();
#endif
...
#ifdef BREATHING_BLINDI_SIZE
        while (this->will_need_breath(large_node_len)){
            this->breath();
        }
#endif
		...
		uint16_t _diff_pos = CalculateBlindiKey(last_small_key, first_large_key);
    ...
    void ShiftLeftLeaf(SeqTreeBlindiNode &left_node, SeqTreeBlindiNode &right_node, unsigned int shift_num) {
#ifdef BREATHING_BLINDI_SIZE
	    while (left_node.will_need_breath(shift_num)) {
            left_node.breath();
	    }
#endif

        // copy the first items from the right node to the last slot in the left node.
        // calculate new blindiKey in the merge slot
        std::copy(right_node.key_ptr, right_node.key_ptr + shift_num, left_node.key_ptr + left_node.valid_len);
        std::copy(right_node.blindiKeys, right_node.blindiKeys + shift_num, left_node.blindiKeys + left_node.valid_len);
        left_node.blindiKeys[left_node.valid_len - 1] =
                CalculateBlindiKey(left_node.key_ptr[left_node.valid_len - 1], left_node.key_ptr[left_node.valid_len]);
        left_node.valid_len += shift_num;

        // shift all slots in the right node to the left
        std::copy(right_node.key_ptr + shift_num, right_node.key_ptr + right_node.valid_len, right_node.key_ptr);
        std::copy(right_node.blindiKeys + shift_num, right_node.blindiKeys + right_node.valid_len, right_node.blindiKeys);
        right_node.valid_len -= shift_num;

#if TREE_LEVELS > 0
        set_blindiTree_from_seq(&left_node);
        set_blindiTree_from_seq(&right_node);
#endif
#ifdef DEBUG_SEQTREE
			std::cout << " shift left left node " << std::endl;
    ...
    void ShiftRightLeaf(SeqTreeBlindiNode &left_node, SeqTreeBlindiNode &right_node, unsigned int shift_num) {
#ifdef BREATHING_BLINDI_SIZE
        while (right_node.will_need_breath(shift_num)) {
            right_node.breath();
        }
#endif

        // shift all slots in the right node
        std::copy_backward(right_node.key_ptr, right_node.key_ptr + right_node.valid_len,
                           right_node.key_ptr + right_node.valid_len + shift_num);
        std::copy_backward(right_node.blindiKeys, right_node.blindiKeys + right_node.valid_len,
                           right_node.blindiKeys + right_node.valid_len + shift_num);
        right_node.valid_len += shift_num;

        // copy the last items from the left node to the first slot in the right node.
        // calculate new blindiKey in the merge slot
        std::copy(left_node.key_ptr + left_node.valid_len - shift_num, left_node.key_ptr + left_node.valid_len,
                  right_node.key_ptr);
        std::copy(left_node.blindiKeys + left_node.valid_len - shift_num, left_node.blindiKeys + left_node.valid_len,
                  right_node.blindiKeys);
        right_node.blindiKeys[shift_num - 1] =
                CalculateBlindiKey(right_node.key_ptr[shift_num - 1], right_node.key_ptr[shift_num]);
        left_node.valid_len -= shift_num;
	...
	void CopyNode(const SeqTreeBlindiNode *node_large) {
		// copy the key_ptr from large node to small_node (this) -> create two trees pointing to small node
		int large_node_len = node_large->get_valid_len();
...
class GenericBlindiSeqTreeNode {
    // Key extractor for blindI
    typedef _KeyExtractor key_extractor_type;
    typedef _KeySerializer key_serializer_type;
    typedef typename _Alloc::template rebind<char>::other byte_alloc_type;

public:
    SeqTreeBlindiNode<_Key, NUM_SLOTS, key_extractor_type, key_serializer_type, _Alloc> node;

    GenericBlindiSeqTreeNode(key_extractor_type &key_extractor, key_serializer_type &key_serializer,
                             byte_alloc_type &alloc) :
            node(key_extractor, key_serializer, alloc) {}
    ...
    typedef typename std::conditional<sizeof(_Key) <= 31,
        ...
        _Key get_key(int pos) const {
            return node.get_key(pos);
        }

        // This was formerly named get_key_ptr
        uint8_t * get_data(int pos) const {
            return node.get_data(pos);
        }

        uint8_t * & get_data_reference(int pos) {
            return node.get_data_reference(pos);
        }

        _Key get_max_key_in_node() const {
            return node.get_key(node.get_valid_len() - 1);
        }
        ...
        _Key get_mid_key_in_node() const {
            return node.get_key(node.get_valid_len() / 2  - 1);
        }

        /*
        * Implements the simplest use-case: search for a key, return its position.
        * If the key was not found, returns NUM_SLOTS (follows the convention of the btree).
        */
        int SearchBlindiNodeWrapper(const uint8_t * search_key) const{

            bool hit = false;

            int position = internal_search_wrapper(search_key,&hit);
        ...
        int PredecessorSearchWrapper(const uint8_t * search_key) const {
            return internal_search_wrapper(search_key);
        }

        int internal_search_wrapper(const uint8_t * search_key, bool * hit = NULL) const {
            bool tmp;
            if (get_valid_len() == 0) {
                return NUM_SLOTS;
            }
            ...
            int position = SearchBlindiNode(search_key, sizeof(_Key), SEARCH_TYPE::PREDECESSOR_SEARCH, &tmp, &smaller_than_node,
                                            tree_traverse, &tree_traverse_len);
            if(hit) *hit = tmp;
            return position;
        }
		...
		// Tree  search
		int SearchBlindiNode(const uint8_t *search_key, uint16_t key_len, SEARCH_TYPE search_type, bool *hit, bool *smaller_than_node, uint16_t *tree_traverse, uint16_t *tree_traverse_len, uint8_t fingerprint = 0, int *diff_pos = NULL, bool *large_small =  NULL) const {
			*smaller_than_node = false;
			bool _large_small = false;
			int _diff_pos = 0;

            // first, we need to serialize the key!
            uint8_t serialized_search_key[sizeof(_Key)];
            node.get_key_serializer().serialize(search_key, serialized_search_key);

            int first_stage_pos = node.SearchFirstStage(serialized_search_key, key_len, tree_traverse, tree_traverse_len);
            ...
            // bring the key from the DB, and serialize it
            _Key extracted_key = node.get_key(first_stage_pos);
            uint8_t *db_key = reinterpret_cast<uint8_t *>(&extracted_key);

            uint8_t serialized_db_key[sizeof(_Key)];
            node.get_key_serializer().serialize(db_key, serialized_db_key);
...
#endif
			_diff_pos = CompareStage (serialized_search_key, serialized_db_key, hit, &_large_small, sizeof(_Key), sizeof(_Key));
			...
			if ((node.get_blindiTree(tree_traverse[2]) == END_TREE) || (_diff_pos < node.get_blindiKey(node.get_blindiTree(tree_traverse[2]))))  { // tree second stage
				...
				return node.SearchSecondStageSequence(first_stage_pos , _diff_pos, _large_small, smaller_than_node, tree_traverse, tree_traverse_len);
			}
        ...
        /**
         * A simple wrapper to support the previous behaviour of blindI insert, which assumes that the data points to the key.
         */
        INSERT_RESULT Insert2BlindiNodeWithKey(uint8_t * insert_key, uint16_t key_len = sizeof(_Key), uint8_t fingerprint = 0) {
            Insert2BlindiNodeWithKey(insert_key, insert_key, key_len, fingerprint);
        }
         ...
         * INSERT TREE
         *
         * @param insert_key pointer to the key
         * @param insert_data the pointer to insert (e.g. pointer to the tuple in the db)
         * @param key_len
         * @param fingerprint
         * @return
         */
		INSERT_RESULT Insert2BlindiNodeWithKey(uint8_t * insert_key, uint8_t * insert_data, uint16_t key_len = sizeof(_Key), uint8_t fingerprint = 0)
		{
#ifdef BREATHING_BLINDI_STSTS 
			insert_count++;
#endif
			...
			INSERT_RESULT return_value = INSERT_SUCCESS;
			...
			if (node.get_valid_len() != 0) { // if the node is not empty
				...
				node.first_insert(insert_data, key_len, fingerprint);
			...
			node.Insert2BlindiNodeInPosition(insert_data, key_len, insert_position, &diff_pos, &large_small, &smaller_than_node, tree_traverse, &tree_traverse_len, fingerprint);
#ifdef BREATHING_BLINDI_STSTS
...
//			}
		...
		void SplitBlindiNode(GenericBlindiSeqTreeNode *node_large) {
			node.SplitBlindiNode(&node_large->node);
#ifdef DEBUG_SEQTREE
			printf("split!!!\n"); 
...
//			}
		...
		void MergeBlindiNodes(GenericBlindiSeqTreeNode *large_node) {
            int small_node_len = this->get_valid_len();
            int large_node_len = large_node->get_valid_len();

            if (node.get_valid_len() == 0) {
                this->node.CopyNode(&large_node->node);
            } else if (large_node->node.get_valid_len() > 0) {
                this->node.MergeBlindiNodes(&large_node->node);
            }

#ifdef BREATHING_BLINDI_SIZE
            large_node->node.deallocate();
#endif
        }

        /**
         * Move `shiftnum` smallest (leftmost) items from `right_node` to `left_node` last slots.
         */
        void ShiftLeftLeaf(GenericBlindiSeqTreeNode &left_node, GenericBlindiSeqTreeNode &right_node, unsigned int shift_num) {
            this->node.ShiftLeftLeaf(left_node.node, right_node.node, shift_num);
        }

        /**
         * Move `shiftnum` biggest (rightmost) items from `left_node` to `right_node` first slots.
         */
        void ShiftRightLeaf(GenericBlindiSeqTreeNode &left_node, GenericBlindiSeqTreeNode &right_node, unsigned int shift_num) {
            this->node.ShiftRightLeaf(left_node.node, right_node.node, shift_num);
        }

		REMOVE_RESULT RemoveFromBlindiNodeWithKey(uint8_t *remove_key, uint16_t key_len = sizeof(_Key), uint8_t fingerprint = 0) {
			bool hit = 0;
			uint16_t position = 0;
			uint16_t tree_traverse[NUM_SLOTS] = {0};
			uint16_t tree_traverse_len;
			...
			return this->RemoveFromBlindiNodeInPosition(position);
		}

        REMOVE_RESULT RemoveFromBlindiNodeInPosition(uint16_t remove_position) {
            REMOVE_RESULT remove_result = node.RemoveFromBlindiNodeInPosition(remove_position);

            int valid_len = node.get_valid_len();
            if (valid_len < NUM_SLOTS / 2 && remove_result == REMOVE_SUCCESS) {
