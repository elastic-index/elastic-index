...
template <typename Key, typename Payload, uint64_t _maxLeafExtries> struct BTreeLeaf : public BTreeLeafBase {
    ...
    typedef GenericBlindiTrieNode<
                TrieBlindiNode<Key, _maxLeafExtries>,
                Key, 
                _maxLeafExtries> 
            blindi_node_t;
...
struct BTree {
  std::atomic<NodeBase *> root;
    ...
    void insert(const Key &k, const Value &v) {
        // fprintf(stderr, "INSERTING ... "); fflush(stderr);
        int restartCount = 0;
    restart:
        if (restartCount++)
            yield(restartCount);
